...
from .service import RuntimeService, now
...
async def terminal_socket(websocket: WebSocket, instance_id: str):
    ...
    try:
        ...
        try:
            terminal = session_factory.scalar(select(models.RuntimeTerminalSession).where(models.RuntimeTerminalSession.token_hash == sha256(raw.encode()).hexdigest(), models.RuntimeTerminalSession.runtime_instance_id == instance_id))
