...
class GradingService:
    ...
    def consume(self, envelope) -> dict:
        ...
        if envelope.event_type not in EVENT_TYPES:
            if envelope.event_type in {"resource.delivery.frozen", "course.roster.frozen"}:
                required_upstream = ["course_id"] + (["class_id"] if envelope.event_type == "course.roster.frozen" else [])
                missing_upstream = [key for key in required_upstream if not payload.get(key)]
                if missing_upstream:self.reject_envelope(envelope,"GRADING.UPSTREAM_FREEZE_PAYLOAD_INVALID","上游冻结事件缺少作用域字段",{"missing":missing_upstream})
    ...
    def precheck(self,course_id,class_id):
        ...
        events=self.repo.grade_events(course_id,class_id)
        checks={"roster_frozen":any(x.action=="UPSTREAM.COURSE_ROSTER_FROZEN" and x.class_id==class_id for x in audits),"gradebook_posted":bool(book and book.status=="POSTED"),"grade_event_traceable":bool(events) and all(x.event_id and x.source_id for x in events),"lab_evidence_referenced":any(x.source_type in {"LAB_CHECKPOINT","LAB_SUBMISSION"} for x in events),"resources_frozen":any(x.action=="UPSTREAM.RESOURCE_DELIVERY_FROZEN" for x in audits)}
    ...
    def freeze_archive(self,course_id,class_id):
        ...
        artifacts=[]
        for kind,content in [("GRADEBOOK_XLSX",self.gradebook_xlsx(course_id,class_id)),("ANALYTICS_XLSX",self.analytics_xlsx(course_id,class_id))]:
