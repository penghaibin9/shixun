...
from .models import (AnalyticsCourseSummary, AnalyticsLabSummary, AnalyticsSectionSummary, AnalyticsStudentLabSummary,
                     AuditEvent, CourseArchive, GradeEvent, Gradebook, GradebookItem, GradingPolicy,
...
class GradingRepository:
    ...
    def grade_events(self, course_id: str, class_id: str | None = None, student_id: str | None = None) -> list[GradeEvent]:
        query = select(GradeEvent).where(GradeEvent.course_id == course_id, GradeEvent.status == "CONSUMED")
        if class_id: query = query.where(GradeEvent.class_id == class_id)
    ...
    def gradebook(self, course_id: str, class_id: str | None = None) -> Gradebook | None:
        query = select(Gradebook).where(Gradebook.course_id == course_id)
        if class_id: query = query.where(Gradebook.class_id == class_id)
    ...
    def archive(self, course_id: str, class_id: str) -> CourseArchive | None:
        return self.session.scalar(select(CourseArchive).where(CourseArchive.course_id == course_id, CourseArchive.class_id == class_id))
    ...
    def risks(self, course_id: str, student_id: str | None = None):
        query = select(StudentRiskFlag).where(StudentRiskFlag.course_id == course_id, StudentRiskFlag.status == "OPEN")
