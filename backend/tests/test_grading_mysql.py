...
MANAGER = {"X-User-Id":"teacher_f","X-Role":"teacher","X-Teacher-Id":"teacher_f","X-Course-Ids":COURSE,"X-Class-Ids":CLASS,"X-Permissions":"grading:manage,audit:read"}
...
def clean_database():
    ...
    with Session(engine) as session:
        ...
        session.execute(delete(DomainEventOutbox));session.execute(delete(FileObject));session.commit()
    yield
    with Session(engine) as session:
        for model in order: session.execute(delete(model))
        session.execute(delete(DomainEventOutbox));session.execute(delete(FileObject));session.commit()
...
def envelope(event_type, student, source, raw, maximum=100, lesson="lesson_3_2"):
    return {"event_id":str(uuid4()),"event_type":event_type,"aggregate_type":"source_fact","aggregate_id":source,"actor_user_id":"upstream","occurred_at":datetime.now(timezone.utc).isoformat(),"idempotency_key":str(uuid4()),"payload":{"course_id":COURSE,"class_id":CLASS,"student_id":student,"lesson_id":lesson,"source_id":source,"raw_score":raw,"max_score":maximum}}
...
def seed_complete_facts(client):
    ...
    for student,scores in {"student_1":[100,80,90,100,70],"student_2":[50,60,70,80,50]}.items():
        for event_type,source,score in zip(["attendance.completed","assignment.submitted","quiz.completed","lab.submitted","poll.completed"],[f"att_{student}",f"ass_{student}",f"quiz_{student}","lab_rsa",f"poll_{student}"],scores):
            ...
            response=client.post("/api/v1/grading/events/consume",headers=MANAGER,json=data);assert response.status_code==200
    return rows


def test_event_replay_three_times_creates_one_grade_event(client):
    data=envelope("quiz.completed","student_1","quiz_once",88)
    responses=[client.post("/api/v1/grading/events/consume",headers=MANAGER,json=data).json() for _ in range(3)]
...
def test_invalid_event_records_one_replayable_error_state(client):
    ...
    responses=[client.post("/api/v1/grading/events/consume",headers=MANAGER,json=data) for _ in range(3)]
...
def test_policy_weight_recalculate_trace_post_analytics_and_student_scope(client):
    ...
    book=client.get(f"/api/v1/gradebook/courses/{COURSE}",headers=MANAGER).json()
    assert [x["total_score"] for x in book["items"]]==[91.0,68.0]
    trace=client.get(f"/api/v1/gradebook/courses/{COURSE}/trace/student_1",headers=MANAGER).json()
    assert trace["total_score"]==91.0 and len(trace["components"])==5
    overview=client.get(f"/api/v1/analytics/courses/{COURSE}/overview",headers=MANAGER).json()
    ...
    section=client.get(f"/api/v1/analytics/courses/{COURSE}/sections/lesson_3_2",headers=MANAGER).json()
    assert section["status"]=="READY" and sum(section["distribution"].values())==2
    labs=client.get(f"/api/v1/analytics/courses/{COURSE}/labs/by-lab",headers=MANAGER).json()
    assert labs["items"][0]["submitted_students"]==2 and labs["items"][0]["avg_score"]==90.0
    posted=client.post(f"/api/v1/grading/courses/{COURSE}/post",headers=MANAGER)
    assert posted.status_code==200 and posted.json()["status"]=="POSTED"
    own=client.get(f"/api/v1/gradebook/courses/{COURSE}/students/student_1",headers=STUDENT)
    denied=client.get(f"/api/v1/gradebook/courses/{COURSE}/students/student_2",headers=STUDENT)
    assert own.status_code==200 and own.json()["items"][0]["total_score"]==91.0
    assert denied.status_code==403
    summary=client.get(f"/api/v1/analytics/courses/{COURSE}/learning-summary",headers=STUDENT,params={"student_id":"student_1"}).json()
    assert summary["status"]=="READY" and summary["grade"]["items"][0]["student_id"]=="student_1"
    assert client.get(f"/api/v1/gradebook/courses/{COURSE}/export.xlsx",headers=MANAGER).content.startswith(b"PK")
    assert client.get(f"/api/v1/analytics/courses/{COURSE}/export.xlsx",headers=MANAGER).content.startswith(b"PK")
...
def test_archive_blocks_until_upstream_facts_then_locks_and_audits(client):
    ...
    client.post(f"/api/v1/grading/courses/{COURSE}/post",headers=MANAGER)
    ...
    for event_type,aggregate in [("course.roster.frozen","roster_2301"),("resource.delivery.frozen","resource_manifest_1")]:
        ...
        assert client.post("/api/v1/grading/events/consume",headers=MANAGER,json=data).status_code==200
    ...
    engine=create_engine(os.environ["YUEKE_DATABASE_URL"])
    with Session(engine) as session: assert session.query(FileObject).count()==2
...
def test_cross_domain_high_risk_audit_is_idempotent_and_immutable(client):
    ...
    results=[client.post("/api/v1/audit/events/ingest",headers=MANAGER,json=data).json()["status"] for _ in range(3)]
...
def test_risk_flags_are_backed_by_checkpoint_and_submission_facts(client):
    for source in ["checkpoint_1","checkpoint_2"]:
        assert client.post("/api/v1/grading/events/consume",headers=MANAGER,json=envelope("lab.checkpoint.failed","student_1",source,0)).status_code==200
    for source in ["lab_1","lab_2","lab_3"]:
        assert client.post("/api/v1/grading/events/consume",headers=MANAGER,json=envelope("lab.submitted","student_2",source,80)).status_code==200
    assert client.post(f"/api/v1/grading/courses/{COURSE}/recalculate",headers=MANAGER,json={"class_id":CLASS}).status_code==200
    risks=client.get(f"/api/v1/analytics/courses/{COURSE}/risks",headers=MANAGER,params={"student_id":"student_1"}).json()["items"]
...
def test_learning_summary_is_pending_without_upstream_facts(client):
    result=client.get(f"/api/v1/analytics/courses/{COURSE}/learning-summary",headers=MANAGER).json()
