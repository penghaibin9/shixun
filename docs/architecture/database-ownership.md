...
| A | `course*`、`class*`、`teaching_*`（含 `teaching_score_proof`）、`attendance_*`、`poll*`、`assignment*`、`quiz*` | 课程、章节、课时和归班事实仅此处创建；`teaching_score_proof` 固化服务端题目冻结、答题与判分证据，教师学生管理必须复用 `class_membership` |
...
A 的 `teaching_score_proof` 与 F 的 `grade_score_proof` 是不同领域的独立事实：前者只由 A 在提交/测验事务内写入，并保存可重算的冻结题集、作答和判分证据；后者只由 F 消费 A 的独立证明事件后写入。A 不写 F 的成绩表或证明表，F 不回写 A 的提交、作答或题目快照。
