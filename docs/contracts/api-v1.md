...
B 的 `lesson_resource` 仅保存实验介绍、文件包、视频和实验定义等资源扩展，不再拥有或决定课时标题、编号、类型与章节。B 的蓝图、就绪度、交付清单、题库模板和审核队列均从 A 的权威目录读取；迁移 `20260922_0013` 在保留旧扩展字段兼容并行内容制作的同时，将历史 49 个课时回填至 A 并建立课程/课时外键。
